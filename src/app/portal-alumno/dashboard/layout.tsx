...
export default function DashboardLayout({
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white font-sans text-gray-900 border-none">
            {/* TOP NAVIGATION BAR (Desktop Only) */}
            <header className="hidden md:block sticky top-0 z-40 w-full bg-white border-b border-gray-100 shadow-sm transition-all">
                            ...
                            <nav className="hidden lg:flex items-center gap-4">
                                {MAIN_LINKS.map((item) => {
                                    ...
                                    return (
                                        <Link
                                            key={item.href}
                                            href={item.href}
                                            className={`flex items-center gap-2.5 px-4 py-2.5 rounded-2xl text-sm font-black transition-all ${
                                                isActive
                                                    ? "text-[#632EB0] bg-purple-50 shadow-sm shadow-purple-100/50"
                                ...
                                <div className="relative">
                                    <button 
                                        ...
                                        className={`flex items-center gap-2.5 px-4 py-2.5 rounded-2xl text-sm font-black transition-all ${
                                            isMoreMenuOpen ? "text-[#632EB0] bg-purple-50 shadow-sm" : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
                                        }`}
                                    >
                                    ...
                                    {isMoreMenuOpen && (
                                        <div className="absolute top-full left-0 mt-3 w-56 bg-white border border-gray-100 shadow-2xl shadow-purple-200/20 rounded-3xl overflow-hidden py-3 z-50 animate-in fade-in slide-in-from-top-2">
                                            {MORE_LINKS.map((item) => {
                                                const isActive = pathname === item.href;
                                                return (
                                                    <Link
                                                        key={item.href}
                                                        href={item.href}
                                                        className={`flex items-center gap-4 px-5 py-3 text-sm font-black transition-colors ${
                                                            isActive ? "text-[#632EB0] bg-purple-50" : "text-gray-600 hover:bg-gray-50"
                                                        }`}
                                                    >
                                                        <item.icon className={`w-4.5 h-4.5 ${isActive ? 'text-[#632EB0]' : 'text-gray-400'}`} />
                                                        {item.name}
                                                    </Link>
                                                )
                                            })}
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            {isUnsubscribed && (
                                <Link 
                                    ...
                                    className="hidden md:flex items-center gap-2 px-6 py-3 bg-[#632EB0] hover:bg-[#4E248B] text-white rounded-[1.25rem] text-sm font-black transition-all shadow-lg shadow-purple-200 active:scale-95"
                                >
                            ...
                            {/* Streak Counter */}
                            <div className="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-orange-50 border border-orange-100 rounded-2xl text-orange-600 font-black text-sm shadow-inner group cursor-default">
                                <Zap className="w-5 h-5 fill-orange-500 group-hover:scale-110 transition-transform" />
                                <span>1 DÍA</span>
                            </div>
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className="flex items-center gap-3 p-1.5 pl-3 pr-4 rounded-2xl hover:bg-gray-50 transition-colors border border-gray-50"
                                >
                                    <div className="w-9 h-9 rounded-xl overflow-hidden bg-gradient-to-br from-purple-100 to-purple-50 border border-purple-200 flex items-center justify-center shrink-0 shadow-sm">
                                       <span className="text-[#632EB0] font-black text-sm">
                                            {userName.charAt(0).toUpperCase()}
                                       </span>
                                    </div>
                                    <div className="flex flex-col items-start leading-none gap-1">
                                        <span className="text-xs font-black text-gray-900 uppercase tracking-tighter">{userName.split(' ')[0]}</span>
                                        <ChevronUp className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-300 ${isProfileMenuOpen ? 'rotate-180' : 'rotate-180'}`} />
                                    </div>
                                </button>

                                {/* Profile Dropdown */}
                                {isProfileMenuOpen && (
                                    <div className="absolute top-full right-0 mt-3 w-72 bg-white border border-gray-100 shadow-2xl shadow-purple-200/20 rounded-[2rem] overflow-hidden p-2 z-50 animate-in fade-in slide-in-from-top-2">
                                        <div className="px-5 py-4 bg-gray-50/50 rounded-2xl border border-gray-100 mb-1">
                                            <p className="text-sm font-black text-gray-900 truncate uppercase tracking-tight">{userName}</p>
                                            <p className={`text-[10px] font-black mt-1 uppercase tracking-widest ${isUnsubscribed ? 'text-orange-500' : 'text-[#632EB0]'}`}>
                                                {isUnsubscribed ? 'Prueba Gratis (Limitado)' : '💎 Suscripción Premium'}
                                            </p>
                                        </div>
                                        <div className="p-1 flex flex-col gap-1">
                                            <Link href="/portal-alumno/dashboard/perfil" className="flex items-center gap-4 px-5 py-3 hover:bg-purple-50 rounded-xl text-sm font-black text-gray-700 transition-colors group">
                                                <User className="w-4.5 h-4.5 text-gray-400 group-hover:text-[#632EB0]" /> 
                                                <span>Monitorear Progreso</span>
                                            </Link>
                                            <Link href="/portal-alumno/dashboard/suscripcion" className="flex items-center gap-4 px-5 py-3 hover:bg-purple-50 rounded-xl text-sm font-black text-gray-700 transition-colors group">
                                                <CreditCard className="w-4.5 h-4.5 text-gray-400 group-hover:text-[#632EB0]" /> 
                                                <span>Facturación y Pagos</span>
                                            </Link>
                                            <Link href="/portal-alumno/dashboard/ajustes" className="flex items-center gap-4 px-5 py-3 hover:bg-purple-50 rounded-xl text-sm font-black text-gray-700 transition-colors group">
                                                <Settings className="w-4.5 h-4.5 text-gray-400 group-hover:text-[#632EB0]" /> 
                                                <span>Ajustes de Cuenta</span>
                                            </Link>
                                        </div>
                                        <div className="h-px bg-gray-100 my-2 mx-5" />
                                        <button onClick={handleSignOut} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-red-50 rounded-2xl text-sm font-black text-red-600 text-left transition-colors">
                                            <LogOut className="w-4.5 h-4.5" /> Cerrar sesión segura
                                        </button>
                                    </div>
                                )}
                            </div>
            ...
            <main className="flex-1 w-full pb-28 md:pb-0">
                <div className="max-w-[1600px] mx-auto pt-0 md:pt-4 pb-4 md:pb-8 px-0 sm:px-6 lg:px-10">
                    {children}
                </div>
            </main>
