...
import { COURSE_DATA, Level, Unit } from '@/constants/courseData';
...
export default function CourseMap() {
  const [activeLevelId, setActiveLevelId] = useState<string>(COURSE_DATA[0].id);
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            // Precision detection: only trigger when a section enters the top focal strip
        ...
        // Thin horizontal strip at 180px down to detect active section like a needle
  ...
  const scrollToLevel = (id: string) => {
    ...
    if (el) {
      const yOffset = -120; // 120px offset for the sticky top header
          ...
          {COURSE_DATA.map((level, idx) => {
            ...
            return (
              <div key={level.id} id={`tab-${level.id}`} className="snap-center shrink-0 flex flex-col items-center gap-2">
                ...
                >
                  {isActive && (
                    <div className="absolute -bottom-[2px] w-6 h-1.5 bg-[#632EB0] rounded-t-full"></div>
                  )}
      ...
      <div className="flex flex-col gap-12 lg:gap-20 py-4 md:py-8 w-full overflow-x-hidden">
        {COURSE_DATA.map((level, idx) => (
          <div key={level.id} id={`level-section-${level.id}`}>
            <LevelSection level={level} index={idx} />
          </div>
        ))}
      </div>
    </>
    ...
    <section className="relative flex flex-col gap-8 md:gap-10">
      ...
      <div className="flex flex-col lg:hidden px-4 md:px-8">
        ...
        <div className="flex flex-col items-center relative w-full pt-4">
          {/* The vertical connection line sitting behind */}
      ...
      <div className="hidden lg:flex flex-col gap-10">
        ...
        {/* Level Header/Title with Icon - Outside the container */}
              ...
              {level.units.map((unit, uIdx) => (
                <CarouselItem key={unit.id} className="pl-4 basis-auto relative">
                  ...
                  <div
                    ...
                    style={{
                      left: uIdx === 0 ? '50%' : '0',
                      right: uIdx === level.units.length - 1 ? '50%' : '0'
                    }}
                  />
                  <UnitCard unit={unit} />
                </CarouselItem>
...
function MobileUnitCard({ unit }: { unit: Unit }) {
      ...
      <div className="flex flex-col flex-1 gap-2 pl-6 md:pl-10">
        ...
        {started && (
          <div className="h-[4px] w-[140px] md:w-[180px] rounded-full bg-gray-100 overflow-hidden mt-1">
            <div 
              className="h-full rounded-full bg-[#21C55D]"
  ...
  return (
    <div className="flex flex-col items-center gap-5 py-2 relative z-10">
      ...
      >
        ...
        {/* Icon */}
        <Icon
          className={`w-16 h-16 transition-transform duration-500 group-hover:scale-125 ${started ? 'text-[#632EB0]' : 'text-gray-300'}`}
        />
        ...
        {/* Progress Bar (at bottom of unit cube) */}
        {started && (
